...
class CrawlerException(PriceDetectorException):
    """크롤러 관련 예외의 기본 클래스"""
    def __init__(self, message: str, error_code: str = "CRAWLER_ERROR", details: dict = None):
...
class CacheException(PriceDetectorException):
    """캐시 관련 예외"""
    def __init__(self, message: str, error_code: str = "CACHE_ERROR", details: dict = None):
...
class DatabaseException(PriceDetectorException):
    """데이터베이스 관련 예외"""
    def __init__(self, message: str, error_code: str = "DB_ERROR", details: dict = None):
...
class InvalidPriceException(ValidationException):
    """유효하지 않은 가격"""
    def __init__(self, price: Any, reason: str, details: dict = None):
        super().__init__("price", f"{reason} (value: {price})", details)


class InvalidURLException(ValidationException):
    """유효하지 않은 URL"""
    def __init__(self, url: str, reason: str, details: dict = None):
...
class BudgetExhaustedException(PriceDetectorException):
    """예산 소진"""
    def __init__(self, remaining_ms: float, details: dict = None):
...
class TimeoutException(PriceDetectorException):
    """타임아웃 예외"""
    def __init__(self, operation: str, timeout_s: float, details: dict = None):
